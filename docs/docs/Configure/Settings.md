...
- `companyName`: display name shown in the sidebar — **required** for the block to take effect
- `logoUrl`: URL of an image to display above the company name — optional
...
- `branding`: disabled (default Spendemon branding)
- `oidc.enabled`: `false`
- `oidc.debug`: `false`
